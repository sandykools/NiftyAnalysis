...
if hasattr(st, 'fragment'):
    print("✅ Fragments are available in this Streamlit version")
else:
    print("❌ Fragments are NOT available. You need Streamlit >= 1.37.0")
    ...
    if "initialized" not in st.session_state:
        ...
        if "nifty_weights" not in st.session_state:
            try:
                ...
                # 🔑 STRICT normalization for Upstox equity symbols
                def normalize_equity_symbol(sym: str) -> str:
                    return (
                        sym.upper()
                        .replace("-", "_")   # BAJAJ-AUTO → BAJAJ_AUTO
                        .replace(" ", "")    # SBI LIFE → SBILIFE
                    )
                ...
                print(f"✅ Loaded {len(st.session_state.nifty_weights)} Nifty weights (normalized)")

            except Exception as e:
                print(f"❌ Failed to load Nifty weights: {e}")
                st.session_state.nifty_weights = {}


            # 🔒 HARD VALIDATION (RUNS ALWAYS)
...
DEFAULT_AUTO_REFRESH_SECONDS = 5  # ⬅️ set to 5 seconds now, increase later if needed
...
# 🔄 UI refresh heartbeat (forces Streamlit rerun)
    ...
    try:
        ...
        try:
            if not nifty_weights:
                ...
                breadth_symbols = list(nifty_weights.keys())
                print(f"DEBUG: Breadth symbols from weights: {len(breadth_symbols)}")
                print(f"DEBUG: First 3 symbols: {breadth_symbols[:3]}")
                
                # Test with a small batch first
                test_symbols = breadth_symbols[:5]  # Just test with first 5
                print(f"DEBUG: Testing with {len(test_symbols)} symbols first")
                
                test_df = client.fetch_equity_quotes(test_symbols)
                print(f"DEBUG: Test fetch result shape: {test_df.shape}")
                
                if not test_df.empty:
                    print(f"DEBUG: Test fetch successful, proceeding with all symbols")
                    equity_quotes_df = client.fetch_equity_quotes(breadth_symbols)
                else:
                    print(f"DEBUG: Test fetch failed, trying alternative approach")
                    # Try without exchange prefix
                    alt_symbols = []
                    for sym in breadth_symbols:
                        if "|" in sym:
                            alt_symbols.append(sym.split("|")[-1])
                        else:
                            alt_symbols.append(sym)
                    
                    print(f"DEBUG: Trying {len(alt_symbols)} alt symbols")
                    equity_quotes_df = client.fetch_equity_quotes(alt_symbols)
                
                print(f"DEBUG: Final equity quotes df shape: {equity_quotes_df.shape if not equity_quotes_df.empty else 'EMPTY'}")
                
                if not equity_quotes_df.empty:
                    print(f"DEBUG: Building constituents dataframe...")
                    constituents_df = build_constituents_df(
                        equity_quotes_df=equity_quotes_df,
                        weights=nifty_weights
                    )
                    
                    print(f"DEBUG: Constituents df shape: {constituents_df.shape}")
                    ...
                    if not constituents_df.empty:
                        ...
                        ccc_value = 0.0
                        print("⚠️ Empty constituents dataframe after building")
                else:
                    ccc_value = 0.0
                    print("⚠️ Empty equity quotes dataframe")
                    constituents_df = pd.DataFrame()
        except Exception as e:
        ...
        # Update CCC history
        try:
            if "ccc_history" not in st.session_state or st.session_state.ccc_history.empty:
                st.session_state.ccc_history = pd.Series([ccc_value], index=[datetime.now()])
            else:
                # Use list concatenation to avoid FutureWarning
                new_data = pd.Series([ccc_value], index=[datetime.now()])
                st.session_state.ccc_history = pd.concat([
                    st.session_state.ccc_history, 
                    new_data
                ]).tail(200)
        except Exception as e:
            print(f"⚠️ Error updating CCC history: {e}")
            st.session_state.ccc_history = pd.Series([ccc_value], index=[datetime.now()])
...
with tab1:
    st.caption(f"🔄 Auto-refresh heartbeat: {st.session_state.refresh_heartbeat}")
    ...
    if hasattr(st, 'fragment'):
        ...
        def test_fragment():
            ...
            with col1:
                if scheduler._last_run:
                    ...
                    st.metric("Scheduler", "⏳ Starting", "Initializing")
        ...
        test_fragment()
        st.caption("👆 This updates every 10 seconds via fragment")
        
    else:
        st.warning("❌ Fragments not available. Using manual refresh.")
    ...
    if st.session_state.force_auto_refresh:
        st.caption("🧪 Debug Mode: Auto-refresh forced ON (ignoring market hours)")
    else:
        st.caption("📈 Production Mode: Auto-refresh runs during market hours only")
    ...
    with col1:
        st.metric(
            "Scheduler",
            "✅ Active" if status["scheduler_active"] else "❌ Inactive"
        )
...
with tab4:
    ...
    available_indices = [
        ...
        # Remove or fix BSE_INDEX - Upstox doesn't support this format
        # "BSE_INDEX|S&P BSE SENSEX"  # REMOVE THIS LINE
    ]
    ...
    available_underlyings = [
        ...
        "FINNIFTY",
        "MIDCPNIFTY",
        # Remove SENSEX if not supported
        # "SENSEX"  # REMOVE THIS LINE
    ]
        ...
        with submit_col2:
            if st.form_submit_button("💾 Save Configuration", width='stretch'):
                ...
                if expiry_date:
                    ...
                    expiry_str = get_trading_expiry(underlying)
                    if not expiry_str:
                        st.error(f"❌ Could not resolve expiry for {underlying}")
                        st.stop()
    ...
    if force_refresh:
        st.warning("⚠️ Debug mode enabled: auto-refresh will run outside market hours")
...
# FINAL STARTUP CHECK
# ==============================
print(f"🎯 Final startup check:")
print("🎯 Final startup check:")
print(f"   - Scheduler executions: {st.session_state.scheduler._total_executions}")
print(f"   - Last scheduler run: {st.session_state.scheduler._last_run}")
print(f"   - Scheduler executions: {st.session_state.scheduler._total_executions}")
print(f"   - Scheduler active: {get_scheduler_status()['scheduler_active']}")
print(f"   - Last run: {st.session_state.scheduler._last_run}")
print(f"   - Option keys cached: {len(st.session_state.option_keys)}")
