...
with st.sidebar:
    ...
    if "background_thread_running" in st.session_state and st.session_state.background_thread_running:
        st.success("✅ Background thread running")
        ...
        st.warning("⚠️ Background thread not running")
        if st.button("▶️ Start Auto-Refresh", width='stretch'):
...
def start_background_manager():
    ...
    print(f"🔍 DEBUG: Attempting to start background manager...")
    print(f"🔍 DEBUG: background_thread_running in session_state: {'background_thread_running' in st.session_state}")
    
    # Check if already running to prevent duplicates
    if "background_thread_running" in st.session_state and st.session_state.background_thread_running:
        print("⚠️ Background manager already running, skipping...")
        return
    
    try:
        # Capture the current script run context
        ctx = get_script_run_ctx()
        print(f"✅ Got ScriptRunContext: {ctx}")
        ...
        def background_worker(context):
            ...
            print("✅ Streamlit context attached to background thread")
            ...
            while True:
                try:
                    ...
                    if "auto_refresh_manager" in st.session_state:
                        ...
                        if should_run:
                            print(f"🔄 Background executing: {reason}")
                            
                            # Execute with retry
                            success, result, msg = manager.execute_with_retry()
                            
                            if success:
                                print(f"✅ Data updated: {msg}")
                                # Update timestamp for frontend to see
                                st.session_state.last_data_update = datetime.now()
                            else:
                                print(f"⚠️ Background execution failed: {msg}")
        ...
        print("✅ Background manager started with context-aware thread")
